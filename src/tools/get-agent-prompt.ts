...
export function registerGetAgentPromptTool(server: McpServer): void {
  server.registerTool(
    "get_agent_prompt",
    {
      description:
        "Retrieve the full markdown system prompt for a specific agent. " +
        "Use the exact filepath returned by search_agents.",
      inputSchema: z.object({
        filepath: z
          .string()
          .describe(
            'The exact repo-relative file path of the agent, e.g. "engineering/database-optimizer.md".'
          ),
      }),
      annotations: {
        readOnlyHint: true,
