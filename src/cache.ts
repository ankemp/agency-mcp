...
let agentCache: AgentDocument[] = [];
let fuseIndex: Fuse<AgentDocument> | null = null;
...
export function getAgentCache(): readonly AgentDocument[] {
  return agentCache;
}

export function getFuseIndex(): Fuse<AgentDocument> | null {
  return fuseIndex;
}
...
async function hydrate(): Promise<void> {
  ...
  if (treeData.truncated) {
    console.error(
      "[hydrate] Warning: tree was truncated by GitHub. Some files may be missing."
    );
  }

  const mdFiles = treeData.tree.filter((node) => {
    if (node.type !== "blob") return false;
    if (!node.path.endsWith(".md")) return false;
    if (IGNORED_PATHS.some((prefix) => node.path.startsWith(prefix))) return false;
    if (IGNORED_FILES.includes(node.path.toLowerCase())) return false;
    return true;
  });
  ...
  console.error(
    `[hydrate] Cache ready — ${agentCache.length} agents indexed.`
  );
}
...
export async function performHydration(): Promise<void> {
  if (hydrating) throw new Error("Hydration is already in progress.");
  hydrating = true;
  hydrationError = null;
  try {
    await hydrate();
