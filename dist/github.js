...
export async function githubFetch(url) {
    ...
    if (res.status === 403) {
        ...
        if (waitMs > 0 && waitMs < 120_000) {
            console.error(`[hydrate] Rate-limited. Waiting ${Math.ceil(waitMs / 1000)}s before retry…`);
            await sleep(waitMs + 1000);
            return fetch(url, { headers: githubHeaders() });
        }
        throw new Error(`GitHub rate limit exceeded. ${PAT ? "Your PAT may have hit its limit." : "Set GITHUB_PAT to raise the limit."}`);
    }
    if (!res.ok) {
